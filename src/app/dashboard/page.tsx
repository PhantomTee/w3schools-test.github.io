    ...
    <AppShell>
      <div className="px-5 sm:px-8 py-8 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-10">
          {[
            { label: 'Live markets',    value: String(liveMarkets.length),               color: 'var(--ink)' },
            { label: 'Total USDC pool', value: `${totalPool.toFixed(0)} USDC`,           color: 'var(--ink)' },
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] px-2 py-0.5 rounded-full bg-[var(--bg-elevated)] text-[var(--ink)]">Connected</span>
                </div>
                {me.xUsername ? (
                  <div className="flex items-center gap-2">
                    <span className="text-[13px] text-[var(--text-muted)]">@{me.xUsername}</span>
                    <span className="text-[11px] px-2 py-0.5 rounded-full bg-[var(--bg-elevated)] text-[var(--ink)]">X verified</span>
                  </div>
                ) : (
                  <div className="flex items-center gap-3">
                    <span className="text-[13px] text-[#64748B]">X not connected</span>
                    <a href="/api/auth/x/connect" className="text-[13px] text-[var(--ink)] hover:text-[var(--text-secondary)] transition-colors">
                      Connect X
                    </a>
                  </div>
                )}
        ...
        <div className="mb-10">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link href="/markets" className="text-[13px] text-[var(--ink)] hover:text-[var(--text-secondary)] transition-colors">
              View all
            </Link>
          </div>
